...
10. problemRestatement、steps 裡的 reason／work、verification、clarificationQuestion、commonMistakes、safetyNote、variationQuestion 這些文字欄位都只能包含最終定稿內容。輸出前先在心裡把 LaTeX 想清楚、想完整，欄位裡絕對不能出現思考過程、自言自語或未完成的草稿（例如表達猶豫、要重新考慮、或要修正前面內容的語氣，包括繁體中文的「不對，」「等一下，」「改為」「換成」「再檢查」這類反覆修正的敘述）；也不能有殘缺、寫到一半的 LaTeX——包括分數或根號等指令的大括號內容是空的、括號或 $ 符號沒有正確配對、指令名稱中間漏掉字母、或指令名稱後面被不該有的空白斷開。單一組 $ ... $ 裡面只能是算式本身，絕對不能把好幾句完整的敘述、或反覆猶豫該用哪個符號的過程，整段塞進同一組 $ 裡。如果某個算式的 LaTeX 太複雜，改用較簡單但正確的等價寫法，不要輸出寫到一半的版本。
...
9. question、keyConcept、difficultyNote 這些文字欄位都只能包含最終定稿內容。在輸出之前，先在心裡把 LaTeX 想清楚、想完整，欄位裡絕對不能出現思考過程、自言自語或未完成的草稿（例如表達猶豫、要重新考慮、或要修正前面內容的語氣，包括繁體中文的「不對，」「等一下，」「改為」「換成」「再檢查」這類反覆修正的敘述）；也不能有殘缺、寫到一半的 LaTeX——包括分數或根號等指令的大括號內容是空的、括號或 $ 符號沒有正確配對、指令名稱中間漏掉字母、或指令名稱後面被不該有的空白斷開。單一組 $ ... $ 裡面只能是算式本身，絕對不能把好幾句完整的敘述、或反覆猶豫該用哪個符號（例如糾結該寫 \\le 還是 \\ne）的過程，整段塞進同一組 $ 裡。如果覺得某個算式的 LaTeX 太複雜，改用較簡單但正確的等價寫法，也不要輸出寫到一半的版本。
...
/**
 * 合法的行內數學式（一組 $ ... $ 之間）只會是算式本身，不會出現中文句子的
 * 標點符號（。？！）或分隔多個子句的逗號／頓號／冒號。螢幕截圖裡出現的
 * 「已知 A = 2^20 × 5^18...其中$1\num \ne 10改為1\node \ne換成1\nle即1...
 * ...LaTeX是 ≤。已知 A = 2^20 \times 5^18...$的值。」就是這個模式：模型把
 * 反覆猶豫「該用 \le 還是 \ne」的整段繁體中文自我修正草稿，連同好幾句完整
 * 敘述，一起錯放進同一組 $ ... $ 裡，而不是只包住定稿後的算式。
 * 這類洩漏不一定會被既有的英文自我修正措辭偵測（hasLeakedDraftArtifacts 裡
 * 的 wait/let's/reconsider 那組規則）攔到，因為草稿內容是繁體中文，所以另外
 * 用「$ 內部出現整句話的標點符號」這個語言無關、只看結構的訊號來偵測。
 */
function hasSentencePunctuationInsideMathDelimiters(text: string): boolean {
  const segments = text.split("$");
  // $ 應該成對出現（偶數個）；奇數索引（1, 3, 5...）代表在 $ 內部的數學式本身。
  for (let i = 1; i < segments.length; i += 2) {
    if (/[。？！，、：]/.test(segments[i])) return true;
  }
  return false;
}

/**
 * 常見的繁體中文「猶豫／自我修正」措辭，對應英文版「Wait, let's fix...」的中文
 * 等價語氣：反覆檢查、否定前一句、把答案從一個值改成另一個值。刻意只挑選
 * 「不太可能出現在正式定稿題目敘述裡」的措辭組合（例如「不對，」「再檢查：」
 * 「讓我」），避免誤傷「換算成」「改為分數」這類正常的數學教學用語。
 */
const CHINESE_DRAFT_HESITATION_PATTERN = /(不對，|不，|再檢查[:：]|讓我(?:再)?想|我(?:需要|再)(?:確認|檢查|想)|抱歉，|等一下，|重新確認|重新想一下|其實應該|正確為\d|不對是\d)/;

/**
 * 偵測 AI 是否把思考草稿、自我修正過程或殘缺 LaTeX 混進最終欄位內容
 * （例如螢幕截圖裡出現的「Wait, let's fix LaTeX in question.」，或反覆猶豫
 * 該用 \le 還是 \ne 的整段繁體中文草稿）。
...
export function hasLeakedDraftArtifacts(text: string): boolean {
  ...
  // 繁體中文版的猶豫／自我修正措辭（見上方常數註解）。
  if (CHINESE_DRAFT_HESITATION_PATTERN.test(text)) return true;
  // $ 應該成對出現，奇數個代表某個數學式沒有正確收尾。
  if (((text.match(/\$/g) ?? []).length) % 2 !== 0) return true;
  // $ 內部出現整句話的標點符號，代表大段敘述（甚至整段草稿）被誤包進數學式的 $ 裡。
  if (hasSentencePunctuationInsideMathDelimiters(text)) return true;
