...
9. question、keyConcept、difficultyNote 三個欄位都只能包含最終定稿內容。在輸出這三個欄位之前，先在心裡把 LaTeX 想清楚、想完整，欄位裡絕對不能出現「Wait」「Let's」「I need to」「重新想一下」之類的思考過程、自言自語或未完成的草稿；也不能有殘缺的 LaTeX（例如空的 \\frac{}{}、沒有配對的括號或 $ 符號、或是 \\times 這類指令名稱被漏字寫成 \\ imes）。如果覺得某個算式的 LaTeX 太複雜，改用較簡單但正確的等價寫法，也不要輸出寫到一半的版本。
10. question 的敘述要精簡：條件較多時，拆成兩到三個短句分別敘述，最後只用一個簡短明確的問句（例如「求 $A+B$ 的值」「$a$ 與 $n$ 分別為何？」）結尾，不要把所有條件、算式、要求硬塞進同一個過長的句子。結尾的問句裡，變數之間一定要有清楚的運算符號或文字連接（例如「$A+B$」「$A$ 與 $B$」），絕對不能出現數字直接黏著變數字母的寫法（例如「1AB」），那代表運算符號或空格被遺漏了。
...
export function hasLeakedDraftArtifacts(text: string): boolean {
  ...
  // LaTeX 指令的反斜線後面絕對不會直接接空白（例如 \times 壞成 "\ imes"、\frac 壞成 "\ frac"）；
  // 出現這個型態代表指令名稱的字元被漏掉或截斷了。
